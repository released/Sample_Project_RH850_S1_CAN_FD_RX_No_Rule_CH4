...
volatile CAN_BUS_PARAMETER_T can_bus_parameter_ch4 = 
{
	...
	#if defined (ENABLE_DATA_RATE_5M)
    // #3
    .NBRP                   = CAN_NBRP_1,
...
const CAN_RX_RULE_TABLE_T tbl_can_bus_rx_rule_ch4[CAN_RX_RULE_CURRENT_AMOUNT] = 
{
    /*
...
void can_rx_fifo_buf_int_check(CAN_REG_TYP * can,CAN_BUS_HANDLE *p, CAN_RX_FIFO_BUFER_NUMBER_e rfi_number)
{
    ...
    if(CAN_REG_READ(rfsts[rfi_number].UINT32,CAN_REG_BIT8,CAN_REG_LENGTH_8)!=0)
    {
        ...
        p->rx_data_ack.bytes |= (0x1<<rfi_number);
        can_bus_parameter_ch4.can_rx_fifo_cnt++;
    }
}
#endif
                          ...
                          const CAN_RX_RULE_TABLE_T *rule)
{
    ...
    //Setting for page 0(0~15) 
    {
        ...
        //RCFDCnCFDGAFLP1_j
        //enable RX FIFO
        p->rrt_handle[q].ptr1.bit.FDPx=(0x1<<can_bus_parameter_ch4.CAN_RX_FIFO_BUFFER_NUM);     //Receive FIFO Buffer 0 Selected(select by bit filed)  / one FIFO buffer depth 128 message
    ...
    can_receive_rule_setting(can,(CAN_CHANNEL_SEL_e)can_bus_parameter_ch4.CAN_CH,CAN_RX_RULE_CURRENT_AMOUNT);
                         ...
                         
{
    ...
    CAN_REG_SET(cst2[channel].CxFDCFG.UINT32,CAN_REG_BIT16,15);   // TDCO[6:0] 
    #endif
...
{    
	can_bus_parameter_ch4.can_rx_isr_cnt++;
...
// void can_run(void)
// {
// 	if(can_bus_parameter_ch4.can_tx1_flag==1)
// 	{
// 		can_bus_parameter_ch4.can_tx1_flag=0;
        
// 		can_tx_buf_data_multi_send(&RCFDC0,&can_bus_handle_ch4.tx_handle[0],can_bus_parameter_ch4.CAN_CH,1,64);
// 	}
...
void can_init(void)
{
    ...
    can_rrt_set(&RCFDC0,&can_bus_handle_ch4,tbl_can_bus_rx_rule_ch4);
    ...
    // can_tx_normal_buffer_set(&can_bus_handle_ch4,can_bus_parameter_ch4.CAN_MODE);
...
/*
    −  Receive buffers RCFDCnCFDRMIDq, RCFDCnCFDRMPTRq, RCFDCnCFDRMFDSTSq, RCFDCnCFDRMDFb_q registers) 
        ...
        RCFDCnCFDRMIDq  <RCFDCn_base> + 2000 H + (80 H × q)  
        RCFDCnCFDRMPTRq  <RCFDCn_base> + 2004 H + (80 H × q)  
        RCFDCnCFDRMFDSTSq  <RCFDCn_base> + 2008 H + (80 H × q)  
        RCFDCnCFDRMDFb_q  <RCFDCn_base> + 200C H + (04 H × b) + (80 H × q)  
...
signed char can_fd_receive_buffer_decode(CAN_REG_TYP * can)
{
    ...
    */
    ...
    {
        tiny_printf("[%d:0x%08X]0x%08X(%d)\r\n",i,&rmnd0[i].UINT32,rmnd0[i].UINT32,rmnd0[i].UINT32);
    }
...
void can_fd_loop_process(void)
{
	...
	if(can_bus_parameter_ch4.can_tx4_flag==1)
	{
		can_bus_parameter_ch4.can_tx4_flag=0;          
        ...
        count += 0x10;
        can_tx_normal_buffer4_set(&can_bus_handle_ch4,can_bus_parameter_ch4.CAN_MODE,p_number,transmit_buffer_id,txbuffer);
		can_tx_buf_data_multi_send(&RCFDC0,&can_bus_handle_ch4.tx_handle[0],can_bus_parameter_ch4.CAN_CH,1,64);

        tiny_printf("CAN TX4 xfer finish\r\n");
	}

	if(can_bus_parameter_ch4.can_tx3_flag==1)
	{
		can_bus_parameter_ch4.can_tx3_flag=0;          
        ...
        count += 0x10;
        can_tx_normal_buffer3_set(&can_bus_handle_ch4,can_bus_parameter_ch4.CAN_MODE,p_number,transmit_buffer_id,txbuffer);
		can_tx_buf_data_multi_send(&RCFDC0,&can_bus_handle_ch4.tx_handle[0],can_bus_parameter_ch4.CAN_CH,1,16);

        tiny_printf("CAN TX3 xfer finish\r\n");
	}

	if(can_bus_parameter_ch4.can_tx2_flag==1)
	{
		can_bus_parameter_ch4.can_tx2_flag=0;          
        
        // prepare data
        transmit_buffer_id = 0x206;
        p_number = 1;

        can_tx_normal_buffer2_set(&can_bus_handle_ch4,can_bus_parameter_ch4.CAN_MODE,p_number,transmit_buffer_id);
		can_tx_buf_data_multi_send(&RCFDC0,&can_bus_handle_ch4.tx_handle[0],can_bus_parameter_ch4.CAN_CH,1,32);
	...
	if(can_bus_parameter_ch4.can_tx1_flag==1)
	{
        ...
        can_tx_normal_buffer1_set(&can_bus_handle_ch4,can_bus_parameter_ch4.CAN_MODE,p_number,transmit_buffer_id);
		can_tx_buf_data_multi_send(&RCFDC0,&can_bus_handle_ch4.tx_handle[0],can_bus_parameter_ch4.CAN_CH,1,64);

        tiny_printf("CAN TX1 xfer finish\r\n");
	}

    if (can_bus_parameter_ch4.can_rx_flag)
    {    


        can_bus_parameter_ch4.can_rx_flag = 0;
    }
}
